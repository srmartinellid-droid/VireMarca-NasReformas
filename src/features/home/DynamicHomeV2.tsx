    ...
    <main style={{ ["--site-accent" as string]: accent }}>
      <section className="relative min-h-[100svh] overflow-hidden bg-[#081A2A] text-white">
        {slides.length ? slides.map((item, index) => (
          <motion.img key={`${item.image}-${index}`} src={item.image} alt={item.alt || "Obra Nascimento Reformas"} className="absolute inset-0 h-full w-full object-cover" initial={{ opacity: 0 }} animate={{ opacity: index === slide ? 1 : 0, scale: index === slide ? 1 : 1.025 }} transition={{ duration: 1.1, ease: "easeInOut" }} />
...
function ServiceCard({ category, index, accent, orange, wa }: { category: ServiceCategory; index: number; accent: string; orange: string; wa: string }) {
  ...
  return <motion.article whileHover={{ y: -4 }} transition={{ duration: .25 }} className="group relative grid min-h-[300px] overflow-hidden border border-stone-200 bg-white shadow-[0_12px_40px_rgba(10,28,43,.06)] sm:grid-cols-[46%_54%]"><span className="absolute left-0 top-0 z-20 h-14 w-1 transition-all duration-500 group-hover:h-20" style={{ backgroundColor: lineColor }} /><div className="relative min-h-[230px] overflow-hidden bg-stone-100 p-4 sm:min-h-full sm:p-5">{category.image ? <div className="h-full overflow-hidden bg-stone-200"><img src={category.image} alt={category.title} className="h-full w-full object-cover object-center transition duration-700 group-hover:scale-[1.012]" /></div> : <div className="grid h-full min-h-[230px] place-items-center p-8 text-center text-sm text-stone-400">Imagem da área ainda não cadastrada</div>}</div><div className="relative flex flex-col justify-between p-7 sm:p-8"><div><div className="flex items-center justify-between"><span className="font-mono text-xs font-medium tracking-[.18em] text-stone-400">{category.number || String(index + 1).padStart(2, "0")}</span><span className="text-xs font-semibold uppercase tracking-[.18em]" style={{ color: lineColor }}>Nascimento</span></div><h3 className="mt-10 text-2xl font-medium leading-tight tracking-[-.035em] text-navy-950 sm:text-[30px]">{category.title}</h3><p className="mt-4 max-w-md text-[15px] leading-7 text-stone-600">{category.description}</p></div><a href={wa} target="_blank" rel="noreferrer" className="mt-7 inline-flex items-center gap-3 text-[11px] font-bold uppercase tracking-[.16em] transition group-hover:gap-4" style={{ color: lineColor }}>Solicitar orçamento <span className="text-base">↗</span></a></div></motion.article>;
}
