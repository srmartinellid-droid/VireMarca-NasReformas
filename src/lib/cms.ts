...
function mergeHomepage(base:HomepageContent,value:Partial<HomepageContent>):HomepageContent{return {...base,...value,hero:{...base.hero,...(value.hero??{})},areas:{...base.areas,...(value.areas??{})},portfolio:{...base.portfolio,...(value.portfolio??{})},method:{...base.method,...(value.method??{})},finalCta:{...base.finalCta,...(value.finalCta??{})}}}
export async function getPublishedProjects(){const supabase=await createClient();const {data}=await supabase.from("projects").select("id,slug,title,description,location,year,cover_image,featured,display_order,categories(name)").eq("published",true).order("featured",{ascending:false}).order("display_order",{ascending:true});return data??[]}
